    ...
    def save(self, episode):
        ...
        for i in range(self.n_agents):
            ...
            torch.save(self.critics_target[i].state_dict(),
                       os.path.join(save_dir, 'critic_target_' + str(i) + '_' + str(episode) + '.pth'))  # 保存目标critic网络的参数

    # 加载模型
    def load(self, episode):
        for i in range(self.n_agents):
            self.actors[i].load_state_dict(
                torch.load('./DRL_Nets2/actor_' + str(i) + '_' + str(episode) + '.pth', map_location=self.device))
            self.actors_target[i].load_state_dict(
                torch.load('./DRL_Nets2/actor_target_' + str(i) + '_' + str(episode) + '.pth', map_location=self.device))
            self.critics[i].load_state_dict(
                torch.load('./DRL_Nets2/critic_' + str(i) + '_' + str(episode) + '.pth', map_location=self.device))
            self.critics_target[i].load_state_dict(
                torch.load('./DRL_Nets2/critic_target_' + str(i) + '_' + str(episode) + '.pth', map_location=self.device))
